...
<<eval=FALSE>>=
kneeflex2 <-stepFlexmix(cbind(RD,1-RD) ~ 1 | N,	model = FLXMRglmfix(family = "binomial", fixed= ~ Th + Sex + Age + Age2), k = 2, nrep = 5, data = knee)

kneeflex3 <-stepFlexmix(cbind(RD,1-RD) ~ 1 | N,	model = FLXMRglmfix(family = "binomial", fixed= ~ Th + Sex + Age + Age2), k = 3, nrep = 5, data = knee)

kneeflex4 <-stepFlexmix(cbind(RD,1-RD) ~ 1 | N,	model = FLXMRglmfix(family = "binomial", fixed= ~ Th + Sex + Age + Age2), k = 4, nrep = 5, data = knee)
@
