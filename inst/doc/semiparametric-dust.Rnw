...
<<fig=TRUE,eval=FALSE>>=
gamdust1 <- gam(bronch ~ s(dust,years), family=binomial, data=dust[(dust$dust<10) & (dust$smoke==1),])
...
<<eval=FALSE>>=
gamdust2<- gam(bronch ~ s(dust) + s(years), family=binomial, data=dust[(dust$dust<10) & (dust$smoke==1),])
@
