...
<<eval=FALSE>>=
travelmode <- matrix(ModeChoice$mode, byrow = T, ncol = 4); colnames(travelmode) = c("air","train","bus","car")
travelhinc <- matrix(ModeChoice$hinc, byrow = T, ncol = 4)
travelhinc <- travelhinc[,1]
travelinvt <- matrix(ModeChoice$invt, byrow = T, ncol = 4); colnames(travelinvt) = c("invtair","invttrain","invtbus","invtcar")
travelgc <- matrix(ModeChoice$gc, byrow = T, ncol = 4); colnames(travelgc) = c("gcair","gctrain","gcbus","gccar")
