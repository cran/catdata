...
###########################
med=glm(ofp ~ hosp+healthpoor+healthexcellent+numchron+age+married+school,family=poisson,data=medmale[learn,])
...
######################
med3=glm.nb(ofp ~ hosp+healthpoor+healthexcellent+numchron+age+married+school,data=medmale[learn,])
...
##################
med4=zeroinfl(ofp ~ hosp+healthpoor+healthexcellent+numchron+age+married+school|1,data=medmale[learn,])
...
cdis <- cdis + pii[j]*exp(-mui[j])*((mui[j]^a[j])/factorial(a[j])) + (1-pii[j])*I(a[j]==0)
a[j] <- a[j]+1
}
a[j]<-a[j] - 1 
}
...
################################
med5=zeroinfl(ofp ~ hosp+healthpoor+healthexcellent+numchron+age+married+school,data=medmale[learn,])
...
####################
med6=hurdle(ofp ~ hosp+healthpoor+healthexcellent+numchron+age+married+school|1,data=medmale[learn,])
...
#######################
med7=hurdle(ofp ~ hosp+healthpoor+healthexcellent+numchron+age+married+school,data=medmale[learn,])
...
names=c("Poisson","Negative\nBinomial","Zero\nInflated 1","Zero\nInflated 2","Hurdle 1","Hurdle 2"),cex=1.3,cex.axis=1.6)
@
...
lrps3[i] <- lrps3[i] + (pnbinom(a,mu=l[j],size=med3$theta) - I(medmale[test,1][j] <= a))^2
#a <- a+1
}
}
